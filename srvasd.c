...
void servidor(int mi_cliente)
{
    ...
    while( ! listo_para_salir ) {
        ...
        if (tag == TAG_PEDIDO) {
			...
			mi_numero = el_mayor;
			faltantes = cant_servidores - 1 ;	//faltan todos los servidores menos yo, empiecen a mandar mensajitos... cant_ser								vidores esta actualizado con la cantidad sin incluir a los finalizados
				if (faltantes == 0) {
		...
		sprintf(buffer,"yo soy el servidor %d y se quiere ir %d ", (mi_cliente-1)/2, origen);
							debug(buffer);
		serv_finalizaron[servidor] = TRUE;
		cant_terminaron++;
		//cant_servidores--;
			//if(!permiso_serv[origen/2]){ 
				//faltantes--;
		//	}
			/*if (faltantes == 0) {	// si el ultimo servidor que faltaba me da permiso, le doy el mutex a mi cliente
							sprintf(buffer,"(me dio el ultimo permiso el que se fue!) le doy el mutex al cliente %d ", (mi_cliente-1)/2);
							debug(buffer);
				    		MPI_Send(NULL, 0, MPI_INT, mi_cliente, TAG_OTORGADO, COMM_WORLD);            
						//debug("le mande el mensaje a mi cliente para que obtenga el mutex ");				
						}
...
*/
		if (cant_terminaron == servidores_comienzo -1 && me_quiero_ir ){
					debug("llega aca");						
					listo_para_salir = TRUE;	
					}
