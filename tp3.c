...
/* Auxiliar para salida de debug (a stdout) */
...
/* Caso particular p/mostrar los params. recibidos */

void debug_params(const t_params *p) {
#ifndef NDEBUG
    assert(mi_rol == ROL_CLIENTE);
    printf("\trk%-3d %c%-3d %c    \t%8dms ini %8dms fin  %4d iters %8dms prev %8dms crit\n",
           mi_rank, 'c', mi_nro, p->caracter,
           p->mseg_computo_inicial, p->mseg_computo_final,
           p->cant_iteraciones,
...
void cliente(int mi_serv_rank, t_params params)
{
    ...
    debug("\tComputando cuestiones iniciales...");
    computar(params.mseg_computo_inicial);
    
    while(params.cant_iteraciones-- > 0) {

        debug("\tComputando parte no crítica...");
    ...
    debug("\tComputando cuestiones finales...");
    computar(params.mseg_computo_final);
...
/* Parsea los primeros 6 strings del arreglo */
...
t_params parsear_primeros_args(char **argv)
{
    ...
    p.caracter = argv[0][0];
    p.mseg_computo_inicial = strtol(argv[1], &ep, 10); assert(!*ep);
    p.mseg_computo_final = strtol(argv[2], &ep, 10); assert(!*ep);
    p.cant_iteraciones = strtol(argv[3], &ep, 10); assert(!*ep);
    p.mseg_computo_previo = strtol(argv[4], &ep, 10); assert(!*ep);
    p.mseg_seccion_critica = strtol(argv[5], &ep, 10); assert(!*ep);
...
char ayuda[] = 
"uso:\n\n"
"  mpiexec -np <#ranks> ./tp3 [<char> <ms_ini> <ms_fin> <#iters> <ms_prev> <ms_crit>] [...]\n\n"
"ej.:\n\n"
"  mpiexec -np 2 ./tp3  a 1000 1000 5 0 100\n"
"  mpiexec -np 4 ./tp3  a 1000 1000 5 0 100   b 500 800 3 1000 100\n\n"
"\n"
...
void parsear_y_repartir_parametros(int argc, char *argv[], int cant_clis)
{
    ...
    pbase.caracter = DEFAULT_CHAR_INICIAL;
    pbase.mseg_computo_inicial = DEFAULT_MSEG_COMPUTO_INICIAL;
    pbase.mseg_computo_final = DEFAULT_MSEG_COMPUTO_FINAL;
    pbase.cant_iteraciones = DEFAULT_CANT_ITERACIONES;
    pbase.mseg_computo_previo = DEFAULT_MSEG_COMPUTO_PREVIO;
    pbase.mseg_seccion_critica = DEFAULT_MSEG_SECCION_CRITICA;
    ...
    for(nro_cli = 0; nro_cli < cant_clis; ++nro_cli) {
        ...
        if(argc > 0) {
            ...
            pcli.caracter = ++pbase.caracter;
            pcli.mseg_computo_inicial = pbase.mseg_computo_inicial;
            pcli.mseg_computo_final = pbase.mseg_computo_final;
            pcli.cant_iteraciones = pbase.cant_iteraciones;
            pcli.mseg_computo_previo = pbase.mseg_computo_previo;
            pcli.mseg_seccion_critica = pbase.mseg_seccion_critica;
        }

        debug("Enviando mensaje inicial con parámetros");
        MPI_Ssend(&pcli, 6, MPI_INT, 2*nro_cli+1, TAG_INICIAL, COMM_WORLD);
    }
}
...
t_params recibir_parametros(void)
{
    ...
    MPI_Recv(&params, 6, MPI_INT, RANK_CERO, TAG_INICIAL, COMM_WORLD, &status);
